---
title: 6. 시민 기술
parent: 한국어
nav_order: 6
---

# 🛠️ 시민 기술
{: .no_toc }

시민이 자신의 전문성으로 사회 문제를 풀어가는 기술. 빠띠는 시민이 데이터를 만들고, 도구를 만들고, 코드를 만드는 모든 활동을 시민 기술(Civic Technology)로 통합해 운영합니다. 데이터는 그중 하나의 형태이고, 그 위에 분석·시각화·시민 프로젝트·시빅해킹이 함께 작동하는 생태계를 만듭니다.

## 목차
{: .no_toc .text-delta }

1. TOC
{:toc}

---

## 플랫폼 기능
{: .text-delta }

시민이 자기 전문성으로 데이터·도구·기술 자산을 만들 수 있는 공간.

### 1. 빠띠 시민기술 플랫폼

✅ 구현 완료
{: .label .label-green }

시민이 데이터를 만들고, 프로젝트를 발의하고, 분석 결과를 공유할 수 있는 빠띠의 시민 기술 플랫폼. 데이터셋·프로젝트·스토리·제안·모임이 한자리에서 연결됩니다.

[빠띠 시민기술](https://data.campaigns.do){: .btn .btn-outline .fs-3 }

### 2. 시민 데이터셋 큐레이션·아카이브

✅ 구현 완료
{: .label .label-green }

시민이 만든 데이터셋을 큐레이션해 누구나 사용할 수 있도록 공개하는 아카이브. 평화의 소녀상 현황, 돌봄노동 통계, 대법원 전원합의체 판례 데이터, 기후위기 국민인식조사, 6411 버스 첫 승객 분석 등 시민이 직접 만들어낸 다양한 공익 데이터셋이 모입니다.

[빠띠 데이터셋](https://data.campaigns.do/datasets){: .btn .btn-outline .fs-3 }

### 3. 시민 데이터 시각화·분석 도구

✅ 구현 완료
{: .label .label-green }

시민이 자신의 데이터를 시각화하고 분석할 수 있는 도구. 코드를 모르는 시민도 의미 있는 통찰을 얻을 수 있도록 설계된 사용자 친화적 인터페이스.

### 4. 빠띠 시민정책 DB

✅ 구현 완료
{: .label .label-green }
⭐ 빠띠 오리지널
{: .label .label-purple }

10년간 빠띠가 운영한 200여 회의 공론장과 시민 캠페인에서 발굴된 시민 정책 제안을 체계적으로 정리한 데이터베이스. 빠띠의 운영 경험 자체가 디지털 공공재가 된 사례.

[빠띠 시민정책 DB](https://data.campaigns.do/datasets/EzqHyg){: .btn .btn-outline .fs-3 }

---

## 이벤트·방법론
{: .text-delta }

시민이 시민 기술을 만들어가는 빠띠의 운영 프로그램.

### 5. 시민 데이터 실험실·시빅해킹 랩

✅ 구현 완료
{: .label .label-green }
⭐ 빠띠 오리지널
{: .label .label-purple }

시민이 직접 데이터를 만들고 분석하며 사회 문제를 풀어가는 빠띠의 실험실 프로그램. 대구 공익데이터실험실, 광주 청소년 "나도 데이터액티비스트" 등 다양한 지역과 대상으로 운영되며, 시민이 시빅해커로 성장하는 핵심 경로.

### 6. 시빅해킹 프로젝트 운영

✅ 구현 완료
{: .label .label-green }

코드포코리아 네트워크와 함께 운영하는 시빅해킹 프로젝트 인큐베이션. 시민이 자기 전문성으로 사회 문제를 발견하고, 그 해결책을 직접 만들어보는 전 과정을 지원합니다.

[코드포코리아](https://codefor.kr){: .btn .btn-outline .fs-3 }

### 7. 데이터 시민 교육·워크숍

✅ 구현 완료
{: .label .label-green }

데이터를 다루고 시각화하며 사회적 의미를 발견하는 시민의 역량을 기르는 교육·워크숍. 빠띠가 다양한 기관·지역과 협력해 운영해 왔습니다.

---

## 대표 프로젝트
{: .text-delta }

시민 기술이 만든 실제 변화의 사례.

### 8. 코로나19 마스크앱·개인안심번호

✅ 구현 완료
{: .label .label-green }
{: .label .label-purple }

2020년 코로나19 대응 과정에서 코드포코리아가 만든 시빅테크의 가장 대표적 사례. 정부의 공적 마스크 데이터를 활용해 시민이 직접 사용할 수 있는 앱을 만들었고, 개인 정보 보호와 공익 사이의 균형을 찾는 개인안심번호 모델을 제시했습니다. 한국이 국제적으로 가장 널리 인정받은 코로나19 시빅테크 대응 사례. 과학기술정보통신부 장관상 수상.

### 9. 안심병원·안심이

✅ 구현 완료
{: .label .label-green }
{: .label .label-purple }

2012년 한국 최초의 시빅해킹 프로젝트 중 하나. 안전한 병원 정보를 시민이 직접 확인할 수 있게 한 도구로, 이후 안심이로 발전했습니다. 한국 시빅테크의 시작 지점 중 하나.

### 10. 시민과 함께하는 도시숲 데이터

✅ 구현 완료
{: .label .label-green }

빠띠와 춘천사회혁신센터, 춘천시청이 함께 만든 도시숲 데이터 프로젝트. 시민이 직접 자기 동네의 도시숲을 관찰하고 데이터를 만들어 도시 정책에 영향을 미친 사례.

### 11. 일상관찰가 우리동네 리어카

✅ 구현 완료
{: .label .label-green }

춘천사회혁신센터와 함께 운영한 시민 관찰 데이터 프로젝트. 시민이 자기 동네의 리어카 사용자를 관찰하고 그 데이터를 통해 보이지 않던 도시 노동의 현실을 드러낸 사례.

### 12. 다양한 시민 데이터셋

✅ 구현 완료
{: .label .label-green }

빠띠 시민기술 플랫폼에는 시민이 만든 다양한 데이터셋이 공개되어 있습니다. 평화의 소녀상 현황, 돌봄노동 통계, 6411 버스 첫 승객 분석, 대법원 전원합의체 판례, 기후위기 국민인식조사, 2030 여성 탄핵집회 인터뷰 등 — 각 데이터셋이 시민이 만든 시빅해킹 프로젝트의 결과물입니다.

[빠띠 시민 데이터셋](https://data.campaigns.do/datasets){: .btn .btn-outline .fs-3 }

---

## 향후 과제
{: .text-delta }

시민 기술의 다음 도약.

### 13. 시민 데이터 신탁 (Citizen Data Trust)

⬜ 향후 과제
{: .label }

시민의 데이터를 시민이 직접 소유·운영하는 신탁 구조. 빅테크가 시민 데이터를 활용해 막대한 수익을 얻는 시대에, 그 데이터의 주권을 시민에게 돌려주고 시민이 함께 결정하는 모델. 빠띠의 시민 데이터 플랫폼과 협동조합 구조가 결합될 때 가능해지는 빠띠 고유의 접근.

### 14. AI 학습용 한국어 공익 데이터셋

⬜ 향후 과제
{: .label }

AI 모델이 학습하는 데이터의 소유와 출처가 점점 중요해지는 시대에, 시민이 직접 만든 한국어 공익 데이터를 AI 학습용으로 정제·공개하는 작업. 빠띠가 가진 시민정책 DB, 공론장 기록, 시민 데이터셋이 그 토대.

---

[← 한국어 메인으로](../){: .btn .btn-outline }
[이전: 거버넌스와 시민의회](../05-governance/){: .btn .btn-outline }
[다음: 플랫폼 빌더와 조직 인프라 →](../07-builder/){: .btn .btn-outline }
